...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //We are saying that if the Display_Value does not contain a decinmal point
        //We want to add a decimal point.
        Calculator.Display_Value += dot;
    }
}

//This section handes operators
function Handle_Operator(Next_Operator) {
    const {First_Operand, Display_Value, operator} = Calculator;
    //When an operator key is pressed we convert the current number
    // displayed on the screen to a number and then sore the results in
    //Calculator.First_Operand if it doesnt already exist.
    const Value_of_Input = parseFloat(Display_Value);
    //Checks if an operator already exists and if Wait_Second_Operand is true,
    //then it updates the operator and exits the function.
...
keys.addEventListener('click', (event) => {
    ...
    if (target.classList.contains('operator')) {
        ...
        Update_Display();
        return; // there wasnt a colon here in the code. i added it.
    }
